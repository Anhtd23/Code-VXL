...
// Ham tao hieu ung Snake_random con moi
void snake(){
	int led = 1; //vi tri led dau tien
	while (led < 8){
		setLed(rand() % 8 , 1); //bat led con moi- con moi random tu tu led dau tien toi led cuoi cung
...
int main(){
	GPIO_Configure();
	while (1){
		int k = 2; //tao so lan chay hieu ung canh buom la 2 lan
		int m = 5; //so lan chay hieu ung snake la 5 lan
		while (k--){
			canh_buom();
		}
		while (m--){
